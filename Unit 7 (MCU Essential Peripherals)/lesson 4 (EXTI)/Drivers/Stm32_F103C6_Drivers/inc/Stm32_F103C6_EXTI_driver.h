...
typedef struct{
	uint16_t				EXTI_Input_lineNumber ;
...
typedef struct{
	EXTI_GPIO_Mapping_t		EXTI_PIN ;					//Specifies the External interrupt GPIO Mapping
									  	  	  	  	   // The parameter must set  based on @Ref_EXTI_Define
	uint8_t					Tregger_case ; 			  //Specifies RISING OR FALLING OR BOTH
													 // The parameter must set  based on @Ref_EXTI_Trigger_Define
	uint8_t					IRQ_Enable ; 			//Enable or Disable the EXTI IRQ (That will enable IRQ mask in EXTI and also on the NVTC Interrupt controller )
	 	 	 	 	 	 	 	 	 	 	       // The parameter must set  based on @Ref_EXTI_IRQ_Define
	void(*P_IRQ_CallBack)(void) ;				  //set the fun() which will be called once the IRQ happened


}EXTI_PinConfig_t;

//-*-*-*-*-*-*-*-*-*-*-*-
//Interrupt Macros Configuration:
//-*-*-*-*-*-*-*-*-*-*-*
...
#endif /* INC_EXTI_DRIVER_STM32F103C6_H_ */
